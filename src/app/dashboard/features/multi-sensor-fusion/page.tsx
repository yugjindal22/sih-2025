...
import { ArrowLeft, Upload, Layers, Satellite, Radio, Thermometer, Image as ImageIcon, Loader2, CheckCircle2, AlertCircle, Combine } from "lucide-react";
...
export default function MultiSensorFusionPage() {
    ...
    const handleImageUpload = (sensorTypeId: string, e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const extractMetadataFromFilename = (filename: string, sensorType: string): SensorImage['metadata'] => {
    ...
    const changeSensorType = (imageId: string, newSensorType: string) => {
        setSensorImages(prev => prev.map(img => 
            img.id === imageId 
                ? { ...img, sensorType: newSensorType, metadata: extractMetadataFromFilename(img.file.name, newSensorType) }
                : img
        ));
    };
    ...
    const parseFusionResponse = (aiResponse: string): FusionResult => {
        ...
        return {
            ...
            sensorComparison: sensorImages.map(img => ({
                sensorType: SENSOR_TYPES.find(s => s.id === img.sensorType)?.name || img.sensorType,
    ...
    const performFusionAnalysis = async () => {
        ...
        setAnalyzing(true);
        try {
            const sensorDescriptions = sensorImages.map((img, idx) => {
                const sensorType = SENSOR_TYPES.find(s => s.id === img.sensorType);
                return `${idx + 1}. ${sensorType?.name} (${img.metadata?.satellite || 'Unknown satellite'}, ${img.metadata?.resolution || 'variable resolution'}, ${img.metadata?.date || 'unknown date'})`;
            }).join('\n');
...
Provide specific, actionable insights about multi-sensor fusion benefits.`;
            ...
            setFusionResult(parseFusionResponse("Error during analysis. Using sample fusion results."));
        } finally {
            setAnalyzing(false);
        }
    };

    const getSensorTypeById = (id: string) => SENSOR_TYPES.find(s => s.id === id);
                ...
                <div className="max-w-7xl mx-auto">
                    <div className="text-center mb-8">
                        ...
                        <p className="text-sm text-muted-foreground">
                            Upload images from different sensors (optical, SAR, thermal) to perform comprehensive multi-sensor fusion analysis
                        </p>
                    </div>
                    ...
                    <Card className="mb-6">
                        <CardHeader>
                            ...
                            <CardDescription>
                                Upload images from different satellite sensors. Minimum 2 sensors required for fusion analysis.
                            </CardDescription>
                        </CardHeader>
                        <CardContent>
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                                {SENSOR_TYPES.map(sensor => (
                                    <div key={sensor.id} className="border rounded-lg p-4">
                                        ...
                                        <input
                                            type="file"
                                            ref={el => fileInputRefs.current[sensor.id] = el}
                        ...
                        <Card className="mb-6">
                            <CardHeader>
                                <div className="flex justify-between items-center">
                                    <div>
                                        <CardTitle>Uploaded Sensors ({sensorImages.length})</CardTitle>
                                        <CardDescription>
                                                ...
                                                <CardContent className="p-4">
                                                    ...
                                                    <div className="space-y-2">
                                                        <Select
                                                            ...
                                                            onValueChange={(value) => changeSensorType(img.id, value)}
                                                        >
                    ...
                    {fusionResult && (
                        <div className="space-y-6">
                            ...
                            <Card>
                                ...
                                <CardContent>
                                    <p className="text-muted-foreground">{fusionResult.summary}</p>
                                </CardContent>
                            </Card>
                            ...
                            <Card>
                                <CardHeader>
                                    ...
                                    <CardDescription>Individual sensor contributions to fusion</CardDescription>
                                </CardHeader>
                                <CardContent>
                                        ...
                                        {fusionResult.sensorComparison.map((comp, idx) => (
                                            <div key={idx} className="border rounded-lg p-4">
                                                ...
                                                <div className="space-y-3">
                                                    ...
                                                    <div>
                                                        <p className="text-sm font-medium mb-2">Unique Features:</p>
                                                        <ul className="text-sm text-muted-foreground space-y-1">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                ...
                                <Card>
                                    <CardHeader>
                                        <CardTitle className="text-lg">Enhanced Detections</CardTitle>
                                    </CardHeader>
                                    <CardContent>
                                        <ScrollArea className="h-[200px]">
                                            <ul className="space-y-2">
                                                {fusionResult.enhancedDetections.map((detection, idx) => (
                                                    <li key={idx} className="flex items-start gap-2">
                            ...
                            <Card>
                                <CardHeader>
                                    ...
                                    <CardDescription>Accuracy gains from multi-sensor fusion</CardDescription>
                                </CardHeader>
                                <CardContent>
                                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                        {fusionResult.confidenceImprovements.map((improvement, idx) => (
                            ...
                            <Card>
                                <CardHeader>
                                    ...
                                    <CardDescription>Optimal sensor fusion strategies</CardDescription>
                                </CardHeader>
                                <CardContent>
                                    <div className="space-y-3">
                        ...
                        <Card>
                            <CardContent className="pt-6">
                                <div className="text-center py-12">
                                    ...
                                    <h3 className="text-lg font-semibold mb-2">Get Started with Multi-Sensor Fusion</h3>
                                    <p className="text-muted-foreground mb-6 max-w-md mx-auto">
                                        Upload images from at least 2 different sensor types to begin fusion analysis.
                                        Combine optical, SAR, thermal, or multispectral data for enhanced insights.
                                    </p>
                                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 max-w-2xl mx-auto">
                                        {SENSOR_TYPES.map(sensor => (
                                            <div key={sensor.id} className="text-center">
                                                <div className={`${sensor.color} text-white p-3 rounded-lg inline-flex mb-2`}>
                                                    {sensor.icon}
                                                </div>
                        ...
                        <Card className="border-amber-500">
                            <CardContent className="pt-6">
                                <div className="flex items-center gap-3">
                                    ...
                                    <p className="text-sm">
                                        Add at least one more sensor image to enable fusion analysis.
                                    </p>
                                </div>
                            </CardContent>
                        </Card>
