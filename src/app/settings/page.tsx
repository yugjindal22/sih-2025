...
const getInitialAdapter = (): "gemini" | "local-oss" => {
  const savedModel = safeLocalStorage("vision_model");
  if (savedModel === "gemini" || savedModel === "local-oss") return savedModel;
  const currentConfig = responseService.getConfig();
  return currentConfig.defaultModel ?? "gemini";
};

const getInitialApiKey = () => safeLocalStorage("gemini_api_key") ?? "";
                ...
                <CardContent className="space-y-4">
                  <div>
                    ...
                    <Input
                      ...
                      value={localOssUrl}
                      onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
                        setLocalOssUrl(e.target.value)
                      }
                      className="mt-2 border-slate-300 dark:border-slate-700"
                    />
                    <p className="text-xs text-slate-600 dark:text-slate-400 mt-2">
                      The URL of your deployed multimodal OSS model (e.g.,
                      InternVL2-2B)
                    </p>
                  </div>
                </CardContent>
