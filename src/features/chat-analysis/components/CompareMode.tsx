...
Provide at least 5-8 specific change categories with quantitative estimates.`;
        ...
        const response = await fetch(
          `https://generativelanguage.googleapis.com/v1beta/models/gemini-3.1-flash-lite-preview:generateContent?key=AIzaSyBRaOsTCMuVDv9xJMB1wJ8NSmoVe9LowZI`,
          {
            method: "POST",
        ...
        <div className="flex items-center justify-between mb-4 sm:mb-6 flex-shrink-0">
          <div className="flex items-center gap-4">
            <div className="relative">
              <div className="absolute inset-0 bg-gradient-primary blur-xl opacity-20 animate-pulse" />
              <div className="relative bg-gradient-primary p-3 rounded-xl shadow-glow">
                <Activity className="w-6 h-6 text-white" />
              </div>
            </div>
            <div>
              <h2 className="text-2xl sm:text-3xl font-bold bg-gradient-to-r from-white via-blue-200 to-purple-300 bg-clip-text text-transparent">
                Change Detection Analysis
              </h2>
              <p className="text-sm text-gray-400 mt-1">AI-Powered Temporal Comparison</p>
            </div>
          </div>
          <div className="flex items-center gap-2">
            <div className="flex gap-1 bg-white/5 p-1 rounded-lg border border-white/10">
              <Button
                ...
                className={viewMode === "slider" ? "bg-white/10" : ""}
              >
              ...
              <Button
                ...
                className={viewMode === "sidebyside" ? "bg-white/10" : ""}
              >
                Side by Side
              </Button>
            </div>
        ...
        <div className="flex-1 grid lg:grid-cols-3 gap-4 sm:gap-6 overflow-hidden min-h-0">
          ...
          <div className="lg:col-span-2 flex flex-col gap-3 sm:gap-4 min-h-0">
            ...
            <div className="flex justify-between gap-4 text-sm flex-shrink-0">
              <div className="flex-1 bg-gradient-to-br from-white/10 to-white/5 backdrop-blur-sm border border-white/20 px-4 py-3 rounded-xl shadow-card overflow-hidden">
                <div className="flex items-center gap-2 min-w-0">
                  <Badge variant="outline" className="bg-blue-500/20 text-blue-300 border-blue-400/30 flex-shrink-0">
                    Before
                  </Badge>
                  <span className="font-medium text-white truncate" title={image1.label}>{image1.label}</span>
                </div>
                {image1.date && (
                  <p className="text-xs text-gray-400 mt-1 truncate" title={image1.date}>{image1.date}</p>
                )}
              </div>
              <div className="flex-1 bg-gradient-to-br from-white/10 to-white/5 backdrop-blur-sm border border-white/20 px-4 py-3 rounded-xl shadow-card overflow-hidden">
                <div className="flex items-center gap-2 min-w-0">
                  <Badge variant="outline" className="bg-purple-500/20 text-purple-300 border-purple-400/30 flex-shrink-0">
                    After
                  </Badge>
                  <span className="font-medium text-white truncate" title={image2.label}>{image2.label}</span>
                </div>
                {image2.date && (
                  <p className="text-xs text-gray-400 mt-1 truncate" title={image2.date}>{image2.date}</p>
                )}
              </div>
            </div>

            {/* Image Viewer */}
            <Card className="bg-white/5 backdrop-blur-sm border-white/10 shadow-card flex-1 min-h-0 overflow-hidden">
              {viewMode === "slider" ? (
                <div
                ...
                >
                  ...
                  {/* Enhanced Slider */}
                  <div
                    className="absolute top-0 bottom-0 w-1 bg-gradient-to-b from-blue-500 via-purple-500 to-pink-500 shadow-glow cursor-ew-resize transition-all group-hover:w-2"
          ...
          <div className="lg:col-span-1 flex flex-col gap-4 min-h-0">
            {/* Summary Card */}
            {isAnalyzing ? (
              <Card className="bg-gradient-to-br from-white/10 to-white/5 backdrop-blur-sm border-white/20 shadow-card p-6">
              ...
              <>
                {/* Summary */}
                <Card className="bg-gradient-to-br from-white/10 to-white/5 backdrop-blur-sm border-white/20 shadow-card p-4 flex-shrink-0">
                  <div className="flex items-start gap-3">
                    <div className="p-2 bg-gradient-primary rounded-lg">
                      <Sparkles className="w-5 h-5 text-white" />
                    </div>
                    <div className="flex-1">
                      <h3 className="text-sm font-semibold text-white mb-1">Analysis Summary</h3>
                ...
                {/* Changes List */}
                <Card className="bg-gradient-to-br from-white/10 to-white/5 backdrop-blur-sm border-white/20 shadow-card flex-1 flex flex-col min-h-0">
